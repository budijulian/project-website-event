...
<!-- style="background: linear-gradient(to bottom right, #fa0000, #b00e0e, #f25c00 100%);" -->
    ...
    <!-- Footer -->
    <footer class="py-5"
        style="background: rgb(63,2,2); margin-top: 120px;
             ...
             // var link;
               $.ajax({
                ...
                dataType : "json",
                    ...
                    var i,kode;
                    if(hasil <= 0){
                        html= "<h4 style='font-size: 14px;  ' class='text-white ml-3 animate__animated animate__zoomIn'>Event Tidak Ditemukan</h4>";
                      }
                     for(i=0; i<hasil.length; i++){
                        // $hasil_replace = preg_replace("![^a-z0-9]+!i", "-", $hasil);
                        var tanggal = hasil[i].dari_tanggal;
